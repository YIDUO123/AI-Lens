/**
 * /admin/analytics · 数据面板 · Suspense 分块版
 * 一屏看:实时今日 · AI 调用 · 内容 Top · 互动汇总 · 订阅漏斗 · 搜索洞察 · 最近事件流
 *
 * 权限:admin/editor · 服务端渲染
 * ⚡ 性能:页面壳立刻出 → 6 个数据区块独立流式加载 → 单块慢不阻塞其他
 * 缓存:每个区块独立 unstable_cache 60s TTL · 二次访问秒返
 */
...
import { db, articles, teardowns, dailyPicks, likes, saves, comments, user, newsItems } from '@/db';
import { sql, desc, eq } from 'drizzle-orm';
import Link from 'next/link';
import { ArrowLeft, Sparkles, Activity, TrendingUp, MessageCircle, Heart, Mail, Search, Zap, ExternalLink } from 'lucide-react';

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

// ============================================================
// 6 个独立缓存函数 · 各自 60s TTL · 单块失败不影响其他
// ============================================================

const getHeroStats = unstable_cache(async () => {
  const [aiOverall, searchStats, subsData, likesCount, savesCount, commentsCount, usersCount] = await Promise.all([
    db.execute(sql`
      SELECT
        COUNT(*)::int AS total,
  ...
  return { aiOverall, searchStats, subsData, likesCount, savesCount, commentsCount, usersCount };
}, ['analytics-hero-v2'], { revalidate: 60, tags: ['analytics'] });

const getAIDistribution = unstable_cache(async () => {
  const [aiByProvider, aiByUseCase] = await Promise.all([
    db.execute(sql`
      SELECT provider,
        COUNT(*)::int AS total,
        COUNT(*) FILTER (WHERE success)::int AS success,
  ...
  return { aiByProvider, aiByUseCase };
}, ['analytics-ai-dist-v2'], { revalidate: 60, tags: ['analytics'] });

const getTopContent = unstable_cache(async () => {
  const [topArticles, topTeardowns] = await Promise.all([
  ...
  return { topArticles, topTeardowns };
}, ['analytics-top-v2'], { revalidate: 60, tags: ['analytics'] });

const getEventsAndStock = unstable_cache(async () => {
  const [eventsByName, picksCount, newsCount, articlesCount, teardownsCount] = await Promise.all([
    db.execute(sql`
    ...
    db.select({ n: sql<number>`count(*)::int` }).from(articles).where(eq(articles.isDraft, false)),
    db.select({ n: sql<number>`count(*)::int` }).from(teardowns),
  ]);
  return { eventsByName, picksCount, newsCount, articlesCount, teardownsCount };
}, ['analytics-events-stock-v2'], { revalidate: 60, tags: ['analytics'] });

const getRecentEvents = unstable_cache(async () => {
  return db.execute(sql`
    SELECT event_name, props, path, created_at
    FROM user_events ORDER BY created_at DESC LIMIT 10
  `);
}, ['analytics-recent-events-v2'], { revalidate: 60, tags: ['analytics'] });

const getRecentAI = unstable_cache(async () => {
  return db.execute(sql`
  ...
  `);
}, ['analytics-recent-ai-v2'], { revalidate: 60, tags: ['analytics'] });

// ============================================================
// 主页面 · 只做 auth 检查 + 页面壳 + 6 个 Suspense 边界
// ============================================================
...
export default async function AnalyticsDashboard() {
  ...
  return (
    <div className="container max-w-6xl py-10 pb-24">
      ...
      <div className="mb-8 flex items-start justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-ink-soft leading-relaxed">
            📊 综合 <b>ai_call_logs · user_events · 业务表</b> 的实时数据 · 6 个区块流式加载 · 想看更细的走势请去
      ...
      {/* Section 4 · 事件分布 + 站内库存 */}
      <Suspense fallback={<TwoColumnSkeleton />}>
        <EventsAndStockSection />
      </Suspense>

      {/* Section 5 · 实时事件流 */}
      <Suspense fallback={<StreamSkeleton title="🌊 实时事件流" />}>
        <RecentEventsSection />
      </Suspense>

      {/* Section 6 · 最近 AI 调用 */}
      <Suspense fallback={<StreamSkeleton title="🤖 最近 AI 调用" />}>
        <RecentAISection />
      </Suspense>
    </div>
  );
...
async function HeroStatsSection() {
  const { aiOverall, searchStats, subsData, likesCount, savesCount, commentsCount, usersCount } = await getHeroStats();
...
async function AIDistributionSection() {
  const { aiByProvider, aiByUseCase } = await getAIDistribution();
  return (
...
async function TopContentSection() {
  const { topArticles, topTeardowns } = await getTopContent();
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
      <section className="bg-cream border-2 border-ink rounded-2xl p-5 shadow-brutal-sm">
...
async function EventsAndStockSection() {
  const { eventsByName, picksCount, newsCount, articlesCount, teardownsCount } = await getEventsAndStock();
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
      <section className="bg-cream border-2 border-ink rounded-2xl p-5 shadow-brutal-sm">
      ...
      <section className="bg-cream border-2 border-ink rounded-2xl p-5 shadow-brutal-sm">
        ...
        <div className="grid grid-cols-2 gap-2">
          <MiniStat label="洞察长文" num={articlesCount[0].n} unit="发布" />
          <MiniStat label="产品拆解" num={teardownsCount[0].n} unit="发布" />
...
async function RecentEventsSection() {
  const eventsRecent = await getRecentEvents();
  return (
...
async function RecentAISection() {
  const aiRecent = await getRecentAI();
  return (
    <section className="bg-cream border-2 border-ink rounded-2xl p-5 shadow-brutal-sm">
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-6 gap-3 mb-8">
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} className="bg-cream border-2 border-ink rounded-xl p-3 shadow-brutal-sm animate-pulse">
          <div className="w-4 h-4 bg-bg-alt rounded mb-2" />
          <div className="h-6 bg-bg-alt rounded mb-1" />
          <div className="h-2 bg-bg-alt rounded w-16" />
        </div>
      ))}
    </div>
  );
...
function TwoColumnSkeleton() {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
      {[0, 1].map((i) => (
        <section key={i} className="bg-cream border-2 border-ink rounded-2xl p-5 shadow-brutal-sm animate-pulse">
          <div className="h-4 bg-bg-alt rounded mb-3 w-40" />
          <div className="space-y-2">
            {Array.from({ length: 4 }).map((_, j) => (
              <div key={j} className="h-8 bg-bg-alt/60 rounded" />
            ))}
          </div>
        </section>
      ))}
...
function StreamSkeleton({ title }: { title: string }) {
  return (
    <section className="bg-cream border-2 border-ink rounded-2xl p-5 shadow-brutal-sm mb-8 animate-pulse">
      <div className="h-4 bg-bg-alt rounded mb-3 w-40" />
      <div className="space-y-1">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="h-6 bg-bg-alt/60 rounded" />
        ))}
      </div>
      <div className="text-[10px] text-muted-foreground mt-2 italic">{title} 加载中…</div>
    </section>
  );
}
